...
export default function TeacherMeetCard() {
  ...
  const handleCreateClass = async () => {
    try {
      ...
      const response = await fetch('/api/teacher/create-meet', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          courseName: selectedCourse?.name || 'Class Meeting',
          description: selectedCourse ? `Meeting for ${selectedCourse.name}` : 'Class meeting'
        })
      });
