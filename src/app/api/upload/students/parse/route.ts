...
export async function POST(request: NextRequest) {
  try {
      ...
      .map((row, index) => {
        ...
        headers.forEach((header, colIndex) => {
          ...
          if (headerLower.includes('name')) {
            ...
            rowData.state = String(value || '').trim();
          } else if (headerLower.includes('district')) {
            rowData.district = String(value || '').trim();
          } else if (headerLower.includes('gender')) {
            rowData.gender = String(value || '').trim();
          }
        });
    ...
    const validData = processedData.filter((row) => {
      ...
      // Role validation - only accept students for this endpoint
      if (row.role !== 'student') {
        errors.push(`Row ${row.rowNumber}: Role must be 'student' for this upload. Found: '${row.role}'`);
        return false;
      }
